    ...
    describe '.get_forecast' do
      it 'returns a forecast for a city' do
        VCR.use_cassette('forecast_facade_spec') do
          ...
          expect(forecast.current_weather[:last_updated]).to eq("2023-04-25 14:15")
          expect(forecast.current_weather[:temperature]).to eq(58.6)
          expect(forecast.current_weather[:feels_like]).to eq(55.7)
